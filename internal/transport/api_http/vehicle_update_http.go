...
// Update http endpoint wrapper
func MakeVehicleUpdateHttpHandler(appCtx appcontext.Context, router *mux.Router, servicesBundle services.Bundle) {
	api := router.PathPrefix("/api").Subrouter()

	endpoint := endpoints.MakeVehicleUpdateEndpoint(appCtx, servicesBundle)
	decoder, _ := MakeVehicleUpdateHttpRequestDecoder(appCtx)
	encoder, _ := MakeVehicleUpdateHttpResponseEncoder(appCtx)

	api.Methods("PUT").Path("/vehicle").Handler(httpTransport.NewServer(
		endpoint,
		decoder,
		encoder,
	))
...
func MakeVehicleUpdateHttpRequestDecoder(appCtx appcontext.Context) (kithttp.DecodeRequestFunc, error) {
	return func(ctx context.Context, r *http.Request) (interface{}, error) {
		var Req interchange.VehicleUpdateRequest
...
func MakeVehicleUpdateHttpResponseEncoder(appCtx appcontext.Context) (kithttp.EncodeResponseFunc, error) {
	return func(ctx context.Context, httpResponse http.ResponseWriter, endpointResponse interface{}) error {
		if endpointResponse != nil {
			response, ok := endpointResponse.(interchange.VehicleUpdateResponse)
			if !ok {
			...
			if contentType == "protobuf" {
				marshaledResponse, err := proto.Marshal(&pb.VehicleUpdateResponse{
					CountUpdated: response.CountUpdated,
				})
				if err != nil {
